...
import { checksum, generateToken, hashToken } from "@/lib/auth/crypto";
import {
  ActorType,
  ApplicationStatus,
  ApplicationStep,
 ...
 * submission fails.
 */
export type FinaliseSubmissionInput = {
  applicationId: string;
  year: number;
...
/**
 * The submission's critical section, as ONE atomic SQL statement.
 *
 * This used to be an interactive transaction: allocate a number from the
 * per-year counter, then update the application, then log the event. That is
 * correct on a local database and fails on a pooled one. Each concurrent
 * submitter held a pooled connection while queuing on the counter's row lock,
 * and through Neon's pooler — ~100 ms per round trip instead of ~1 ms — the
 * queue outlasted Prisma's transaction start timeout. Submissions failed with
 * "Unable to start a transaction in the given time".
 *
 * A single statement fixes that without giving anything up:
 *
 *  - `target` locks the application row only if it is still a draft;
 *  - `counter` increments the year's counter only if `target` found a row, so
 *    a double submit, or an application already submitted, never burns a
 *    number — the guarantee a plain Postgres sequence could not give;
 *  - `updated` writes the reference and submission fields;
 *  - `event` records the submission in the applicant-visible history.
 *
 * The row lock is held for the statement alone — microseconds — and nothing
 * waits on a JavaScript round trip while holding it. Lock order (application,
 * then counter) is identical for every submitter, so there is no deadlock.
 *
 * Returns the reference, or null if the application was no longer a draft.
 */
export async function finaliseSubmission(
  input: FinaliseSubmissionInput,
  client: Pick<Prisma.TransactionClient, "$queryRaw"> = db,
): Promise<string | null> {
  const eventId = randomUUID();

  const rows = await client.$queryRaw<{ referenceNumber: string }[]>`
    WITH target AS (
      SELECT "id"
      FROM "applications"
    ...
    counter AS (
      ...
      SELECT ${input.year}, 1, now() FROM target
    ...
    updated AS (
      ...
      SET "status" = 'SUBMITTED'::"ApplicationStatus",
          "currentStep" = 'REVIEW'::"ApplicationStep",
          "referenceNumber" = ${REFERENCE_PREFIX} || '-' || ${input.year}::text
                              || '-' || lpad(counter."lastNumber"::text, 6, '0'),
      ...
      RETURNING a."id", a."referenceNumber"
    ),
    event AS (
      INSERT INTO "application_events"
        ("id", "applicationId", "action", "actorType", "newValue", "isVisibleToApplicant", "createdAt")
      SELECT ${eventId}, updated."id", 'submitted', 'APPLICANT'::"ActorType",
             'SUBMITTED', true, now()
      FROM updated
    )
    SELECT "referenceNumber" FROM updated
  `;

  return rows[0]?.referenceNumber ?? null;
}
...
export async function submitApplication(
  ...
  const referenceNumber = await finaliseSubmission({
    applicationId: application.id,
    year,
    accessTokenHash: hashToken(accessToken),
    contactEmail: application.guardianEmail ?? null,
    contactPhone: application.guardianPhone ?? null,
  });
