...
const pages = [
  ...
  "/settings", "/navigation", "/outbox", "/users", "/audit", "/account", "/search?q=school",
 ...
 */
export async function runA11y(session) {
...
try {
  for (const { app, url } of pages) {
    for (const vp of VIEWPORTS) {
      ...
      if (ADMIN_SESSION) {
        await page.setCookie({ name: "bass_session", value: ADMIN_SESSION, domain: new URL(BASE).hostname, path: "/" });
      }
