...
import { ApplicationStatus, ApplicationStep, StudyLevel } from "@bass/db/enums";
  ...
  it("hands out consecutive, unique numbers under concurrent submissions", async () => {
    const references = await Promise.all(
      Array.from({ length: 25 }, () =>
        db.$transaction((tx) => allocateReferenceNumber(tx, TEST_YEAR)),
      ),
    );

    const unique = new Set(references);
    assert.equal(unique.size, 25, "every reference must be distinct");

    const sequences = references.map((ref) => Number(ref.split("-")[2])).sort((a, b) => a - b);
    assert.deepEqual(
      sequences,
      Array.from({ length: 25 }, (_, index) => index + 1),
      "the sequence must have no gaps and no repeats",
    );
    assert.match(references[0]!, new RegExp(`^BASS-${TEST_YEAR}-\\d{6}$`));
  });

  it("is rolled back with the transaction it runs in", async () => {
    const before = await db.applicationCounter.findUnique({ where: { year: TEST_YEAR } });

    await assert.rejects(
      db.$transaction(async (tx) => {
        await allocateReferenceNumber(tx, TEST_YEAR);
        throw new Error("submission failed after taking a number");
      }),
    );

    const after = await db.applicationCounter.findUnique({ where: { year: TEST_YEAR } });
    assert.equal(after?.lastNumber, before?.lastNumber, "a failed submission must not burn a number");
  });
});
